...
// voters/reporters = 投票/通報済みアカウント(subのハッシュ)のJSON配列。CacheServiceは揮発（TTL前でも退去・最長6h）
// なので、二重投票/二重通報の抑止をシートに永続化する（「1アカウント1票」を時間が経っても保証する）。
var HEADERS         = ['timestamp', 'status', 'id', 'type', 'cat', 'json', 'q', 'src', 'device', 'reports', 'up', 'voters', 'reporters'];
...
var RATE_PER_HOUR   = 20;                       // 投稿レート上限/時（キー＝本人確認済みは sub・匿名は device）
var ANON_PER_HOUR   = 30;                       // 匿名(本人確認なし)投稿の「全体」上限/時。device はクライアントが
                                                //   自由に名乗れる＝毎回変えれば device 単位レートは無限回避できるため、
                                                //   匿名分には端末非依存の総量クォータを併設（pending洪水→投稿停止DoSを防ぐ）
var TOKENINFO_PER_HOUR = 500;                   // tokeninfo(UrlFetchApp) 呼び出しの全体予算/時。検証結果はトークン単位で
                                                //   キャッシュするので正規利用では到達しない。悪意の連打で GAS の
                                                //   UrlFetch 日次クォータを枯渇させ「本人確認の静かな死」を誘発する攻撃の歯止め
...
var MAX_DEDUP_CELL  = 1000;  // voters/reporters セルに保持するアカウントハッシュ数の上限（セル50K文字制限の保護）
...
var COL = { ts:0, status:1, id:2, type:3, cat:4, json:5, q:6, src:7, device:8, reports:9, up:10, voters:11, reporters:12 };
...
function doGet(e) {
  try {
    ...
    try { console.error('doGet error: ' + (err && err.stack || err)); } catch (_ig) {}
    return json_({ ok: false, error: 'server error', questions: [] }); // 内部メッセージは外に出さない（ログのみ）
  }
}
...
function doPost(e) {
  try {
    ...
    var sub = idtoken ? verifyTokenSub_(idtoken) : '';   // 検証済みアカウントID（レート制限のキーにも使う）
    var trusted = !!sub;
    ...
    try {
      ...
      var devH = hash_(device);   // シートには device の生値でなくハッシュを保存（漏えい時の評判なりすまし対策）
      if (lastRow >= 2) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          // 旧行は生 device、新行はハッシュ。両形式と照合（移行で承認実績が消えないように）
          if (st === 'approved' && (String(row[COL.device]) === devH || String(row[COL.device]) === device)) rep++;
        }
        // pending滞留の上限は「匿名」だけを止める：device偽装の洪水で本人確認済みユーザーまで投稿不能にさせない
        if (!trusted && pending >= MAX_PENDING) return json_({ ok: false, error: 'too many pending' });
      }
      ...
      newRow[COL.device]  = devH;   // 生deviceは保存しない（ハッシュのみ）
      newRow[COL.reports] = 0;
      newRow[COL.up]      = 0;
      newRow[COL.voters]    = '';
      newRow[COL.reporters] = '';
    ...
    try { console.error('doPost error: ' + (err && err.stack || err)); } catch (_ig) {}
    return json_({ ok: false, error: 'server error' }); // 内部メッセージは外に出さない（ログのみ）
  }
}
  ...
  try {
    ...
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]) === id) {
        ...
        // 永続の二重通報抑止：reporters 列（subハッシュのJSON配列）。CacheService は揮発（最長6h）なので、
        // 時間を置いた同一アカウントの再通報でも reports を二重加算させない（良問のじわじわ降格を防ぐ）
        var rH = hash_(reporter);
        var rlCell = sh.getRange(rowNum, COL.reporters + 1);
        var rl = parseJsonList_(rlCell.getValue());
        if (rl.indexOf(rH) >= 0) return json_({ ok: true, status: 'already' });
        var stCell = sh.getRange(rowNum, COL.status + 1);
        var st = String(stCell.getValue()).trim().toLowerCase();
        if (st !== 'approved') return json_({ ok: false, error: 'not found' }); // 降格は approved のみ対象。pending への事前通報の仕込み（公開と同時に降格）も防ぐ
        var rpCell = sh.getRange(rowNum, COL.reports + 1);
        var n = parseInt(rpCell.getValue() || '0', 10) + 1;
        rpCell.setValue(n);
        rl.push(rH); rlCell.setValue(JSON.stringify(rl.slice(-MAX_DEDUP_CELL)));
        if (n >= REPORT_LIMIT) {
...
function handleVote_(id, idtoken) {
  ...
  try {
    ...
    if (c) {
      ...
      if (c.get(dk)) return json_({ ok: true, status: 'already' }); // 同一アカウント×同一問題の二重投票は無視（成功扱い）
    }
  } catch (e) { /* キャッシュ不可でも続行 */ }
  ...
  try {
    ...
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]) === id) {
        var rowNum = i + 2;
        try { var cR = CacheService.getScriptCache(); if (cR) { if (cR.get(dk)) return json_({ ok: true, status: 'already' }); var rnL = parseInt(cR.get(rk) || '0', 10); if (rnL >= VOTE_RATE_HR) return json_({ ok: false, error: 'rate limited' }); rn = rnL; } } catch (_r) {} // ロック内で再確認＝read-modify-writeを直列化（同一アカウントの同時押しによる二重加算/レート超過を遮断・TOCTOU封じ）
        // 永続の二重投票抑止：voters 列（subハッシュのJSON配列）。CacheService は揮発（最長6h）なので、
        // 時間を置いた同一アカウントの再投票でも up を水増しさせない（✓検証済みバッジの信頼性の根拠）
        var vH = hash_(voter);
        var vlCell = sh.getRange(rowNum, COL.voters + 1);
        var vl = parseJsonList_(vlCell.getValue());
        if (vl.indexOf(vH) >= 0) return json_({ ok: true, status: 'already' });
        var st = String(sh.getRange(rowNum, COL.status + 1).getValue()).trim().toLowerCase();
        if (st !== 'approved') return json_({ ok: false, error: 'not found' }); // pending中の自分の投稿に👍を仕込み公開と同時に✓検証済みへ昇格させる細工を防ぐ
        var upCell = sh.getRange(rowNum, COL.up + 1);
        var n = parseInt(upCell.getValue() || '0', 10) + 1;
        upCell.setValue(n);
        vl.push(vH); vlCell.setValue(JSON.stringify(vl.slice(-MAX_DEDUP_CELL)));
...
// 投稿の簡易レート制限（CacheService、1時間窓）。キー＝検証済みsub('s_…') or 匿名device('d_…')。
function rateOk_(key) {
  try {
    var c = CacheService.getScriptCache(); if (!c) return true;
    var k = 'rl_' + key, n = parseInt(c.get(k) || '0', 10);
    if (n >= RATE_PER_HOUR) return false;
    c.put(k, String(n + 1), 3600);
    return true;
  } catch (e) { return true; }
}
// 匿名投稿の全体クォータ（1時間窓）。device は名乗り放題なので、匿名分は端末非依存の総量で頭打ちにする。
function anonOk_() {
  try {
    var c = CacheService.getScriptCache(); if (!c) return true;
    var n = parseInt(c.get('anonq_hr') || '0', 10);
    if (n >= ANON_PER_HOUR) return false;
    c.put('anonq_hr', String(n + 1), 3600);
    return true;
  } catch (e) { return true; }
}
// tokeninfo(UrlFetchApp) 呼び出しの全体予算（1時間窓）。超過時は本人確認をフェイルクローズで縮退。
function fetchBudgetOk_() {
  try {
    var c = CacheService.getScriptCache(); if (!c) return true;
    var n = parseInt(c.get('tokfetch_hr') || '0', 10);
    if (n >= TOKENINFO_PER_HOUR) return false;
    c.put('tokfetch_hr', String(n + 1), 3600);
    return true;
  } catch (e) { return true; }
}
// SHA-256 の先頭8バイトを hex 化（sub/device/トークンの匿名化キー。シートに生の識別子を残さない）
function hash_(s) {
  var d = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, String(s == null ? '' : s), Utilities.Charset.UTF_8);
  var h = '';
  for (var i = 0; i < 8; i++) { var b = (d[i] + 256) % 256; h += ('0' + b.toString(16)).slice(-2); }
  return h;
}
// voters/reporters セル（JSON配列）の安全な読出し。壊れ/空/非配列は []
