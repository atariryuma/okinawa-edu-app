...
// idtoken は JWT形式（header.payload.signature）にする＝verifyTokenSub_ の形式事前チェックを通すため（実トークンと同じ3パート）
const voteAs = (id, sub) => { validToken(); _tokeninfo.sub = sub; return post({ action:'vote', id, idtoken:'h.'+sub+'.s' }); }; // 検証済み sub ごとに1票（端末ではなくGoogleアカウントで集計）
const reportAs = (id, sub) => { validToken(); _tokeninfo.sub = sub; return post({ action:'report', id, idtoken:'h.'+sub+'.s' }); }; // 通報も sub 基準（端末ID偽装での良問降格を防ぐ）
...
section('verifyToken_: aud/azp/email_verified/iss/exp の全ゲート');
{
  ...
  validToken(); ok('全条件OK→true', GAS.verifyToken_('h.ok.s') === true);
  validToken(); _tokeninfo.aud = 'other'; ok('aud不一致→false', GAS.verifyToken_('h.aud.s') === false);
  validToken(); _tokeninfo.azp = 'other'; ok('azp不一致→false（aud偽装の素通し封じ）', GAS.verifyToken_('h.azp.s') === false);
  validToken(); _tokeninfo.email_verified = 'false'; ok('email未確認→false', GAS.verifyToken_('h.email.s') === false);
  validToken(); _tokeninfo.iss = 'evil.com'; ok('iss不正→false', GAS.verifyToken_('h.iss.s') === false);
  validToken(); _tokeninfo.exp = String(Math.floor(Date.now()/1000) - 10); ok('失効→false', GAS.verifyToken_('h.exp.s') === false);
  validToken(); delete _tokeninfo.exp; ok('exp欠落→false（フェイルクローズ＝未失効を勝手に真としない）', GAS.verifyToken_('h.noexp.s') === false);
  _tokeninfo = null; ok('tokeninfo 200以外→false', GAS.verifyToken_('h.http400.s') === false);
  // verifyTokenSub_：検証OKなら sub を返す／NGなら空文字（投票の本人確認に使用）
  validToken(); ok('verifyTokenSub_ 正常→subを返す', GAS.verifyTokenSub_('h.sub.s') === 'sub-default');
  validToken(); _tokeninfo.sub = ''; ok('sub欠落→空（不可）', GAS.verifyTokenSub_('h.nosub.s') === '');
  validToken(); _tokeninfo.aud = 'other'; ok('aud不一致→空', GAS.verifyTokenSub_('h.aud2.s') === '');
...
section('verifyTokenSub_: キャッシュで同一トークンの連打を1回のfetchに畳む／予算超過はフェイルクローズ');
{
  ...
  ok('初回はfetchする', GAS.verifyTokenSub_('h.cacheA.s') === 'sub-default' && _fetchCount === before + 1);
  _tokeninfo = null;   // 以降 tokeninfo が死んでいても…
  ok('2回目はキャッシュ（fetchしない・結果同一）', GAS.verifyTokenSub_('h.cacheA.s') === 'sub-default' && _fetchCount === before + 1);
  ok('無効トークンもネガティブキャッシュ', GAS.verifyTokenSub_('h.cachebad.s') === '' && _fetchCount === before + 2 && GAS.verifyTokenSub_('h.cachebad.s') === '' && _fetchCount === before + 2);
  // 予算超過＝tokeninfo を叩かずフェイルクローズ（UrlFetch日次クォータの枯渇＝「本人確認の静かな死」を攻撃で誘発させない）
  validToken(); _cache['tokfetch_hr'] = '500';
  const b2 = _fetchCount;
  ok('予算超過で検証はfetchせず失敗（フェイルクローズ）', GAS.verifyTokenSub_('h.cachebudget.s') === '' && _fetchCount === b2);
  delete _cache['tokfetch_hr'];
}
section('verifyTokenSub_: JWT形式でないトークンはfetch前に弾く（安価な予算枯渇DoSの遮断）');
{
  reset(); validToken();
  const b0 = _fetchCount;
  ok('ドット無しゴミ文字列はfetchしない', GAS.verifyTokenSub_('garbage-no-dots') === '' && _fetchCount === b0);
  ok('2パート（ドット1つ）もfetchしない', GAS.verifyTokenSub_('only.one') === '' && _fetchCount === b0);
  ok('不正文字(!)を含む3パートもfetchしない', GAS.verifyTokenSub_('a.b!.c') === '' && _fetchCount === b0);
  ok('予算カウンタも消費しない（ゴミ連打で枯渇しない）', !_cache['tokfetch_hr']);
  ok('正しいJWT形式は通常通り検証する（fetchする）', GAS.verifyTokenSub_('aGVhZGVy.cGF5bG9hZA.c2ln') === 'sub-default' && _fetchCount === b0 + 1);
}
...
reset(); validToken();
ok('② ログイン+出典あり → approved', post({ q:Q('a2','地公法29条'), idtoken:'h.tk.s', device:'dB' }).status === 'approved');
reset(); validToken();
ok('③ ログイン+出典なし → pending（出典ゲート）', post({ q:Q('a3','自作メモ'), idtoken:'h.tk.s', device:'dC' }).status === 'pending');
reset(); validToken();
post({ q:Q('r1','学校教育法第37条'), idtoken:'h.tk.s', device:'dR' });
post({ q:Q('r2','地公法29条'), idtoken:'h.tk.s', device:'dR' });   // dR の承認実績=2
_tokeninfo = null;                                              // 以降は匿名
ok('④ 匿名+出典あり+実績2 → approved（段階信頼）', post({ q:Q('r3','学校保健安全法第19条'), idtoken:'', device:'dR' }).status === 'approved');
reset(); validToken();
post({ q:Q('dup','学校教育法第37条'), idtoken:'h.tk.s', device:'dD' });
ok('⑤ 重複id → 拒否', post({ q:Q('dup','学校教育法第37条'), idtoken:'h.tk.s', device:'dD' }).ok === false);
reset();
...
reset(); validToken();
post({ q:Q('p1','学校教育法第37条'), idtoken:'h.tk.s', device:'d1' });
post({ q:Q('p2','地公法29条'), idtoken:'h.tk.s', device:'d2' });
...
reset(); validToken();
post({ q:Q('rp','学校教育法第37条'), idtoken:'h.tk.s', device:'d1' });
...
reset(); validToken();
post({ q:Q('real','学校教育法第37条'), idtoken:'h.tk.s', device:'dG' });   // approved な実在問題
{
...
reset(); validToken();
post({ q:Q('forge','学校教育法第37条'), idtoken:'h.tk.s', device:'d1' });   // approved
{
...
{
  reset(); validToken();
  post({ q:Q('pv','学校教育法第37条'), idtoken:'h.owner.s', device:'d1' });   // approved
  voteAs('pv', 'sX');
...
section('doPost: device偽装でレートを回避してもpending洪水でサービスを止められない');
{
  ...
  validToken();
  ok('本人確認済みは匿名クォータの影響を受けない', post({ q:Q('tr1','学校教育法第37条'), idtoken:'h.trusted1.s', device:'dOK' }).status === 'approved');
  reset();
  ...
  validToken();
  ok('本人確認済みはpending洪水でも投稿できる（DoS耐性）', post({ q:Q('tr2','地公法29条'), idtoken:'h.trusted2.s', device:'dOK2' }).status === 'approved');
}
...
{
  reset(); validToken();
  post({ q:Q('dh1','学校教育法第37条'), idtoken:'h.dh.s', device:'dRaw' });
